...
# 1. СПОСОБ - Самый простой - метод yeild. Он вызывает переданный блок
  ...
  # yield может быть вызван и так
  # ПРИМЕР! PROGS\tmp\oop_basics\Task_7\depot_v3\interface.rb
  def station_details                             # метод принимает блок и проходит по всем поездам на станции, передавая каждый поезд в блок.
    @trains.each{ |train| yield(train) }                # yield - вызываем блок, который передается методу station_details при его вызове в интерфейсе. В переменную train попадает поезд из массива @trains
  end
