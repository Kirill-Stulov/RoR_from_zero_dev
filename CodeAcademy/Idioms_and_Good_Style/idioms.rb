...
# ИДИОМА №2 'Множественное присваивание'
  ...
  # Обменять значение двух переменных не при, не прибегая к третьей переменной
  x, y = 1, 2
  # => 1, 2
  x, y = y, x
...
# ИДИОМА 7 часто к массиву применяем итератор и некий метод (16:55)
  ...
  girls.each(&:upcase)
  # т.е если есть некий массив или хеш и некий итератор (map, each...), т.е перебираем каждый элемент массива и к каждомк применяем один и тот-же метод (18:15)
...
# ИДИОМА 10 "переопределение дефолтного поведения" Допустим хотим чтобы при присвоении имени пользователю, оно сразу автоматически начиналось с заглавной буквы (27:00)
  ...
  class User2
    ...
    def name=(value)                      # переписанный сеттер
      @name = value.capitalize               # переписываем сеттер таким образом, что инстанс переменной @name (она совпаает по имени с тем что в attr_accessor) присваиваем value.capitalize 
...
# ИДИОМА 12 "передача переменного количества аргументов в метод"- еще одна возможность чистого ruby (30:00)                                      
  ...
  #   @number = number                                                                   # дальше это все инициализируем
  #   @type = type
  #   @model = model
