# совмещаем методы класса и методы экземпляра (instance методы) в одном модуле (Debagger)
...
# какие методы включить как методы класса, а какие как инстанс методы 0:34:55
# ПРИМЕНЕНИЕ ЭТОГО В СЛЕДУЮЩЕМ ФАЙЛЕ "class_and_instance_methods_in_module2.rb"																			


class Car
	include FuelTank                                                  					#!!! extend включает методы которые есть в модуле как методы класса
	extend Debagger:: ClassMethods  		#4. Теперь можно сделать так - подключаем 	#!!! include подключает методы которые есть в модуле как инстанс методы
