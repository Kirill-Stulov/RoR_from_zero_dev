...
class Car
	...
	NUMBER_FORMAT = /^[а-я]{1}\d{3}[а-я]{2}/i # ^ означает что данный шаблон должен применяться с начала строки (50:28; 52:30) 

	def initialize(number)          #3 вызываем обязательный параметр number при инициализации
		@number = number             # присваиваем этот параметр в переменную, т.к в конструкторе не можем обращаться к геттерам/сеттерам
		@current_rpm = 0
	...
	def valid?                      #6 добавили публичный метод valid? для обращения к защищенному методу validate!   Метод valid? возвращает либо true, либо false
		validate!
		true
	rescue							# в случае если validate! выбросит исключение, возвращаем false 00:54:40
		false
	end
