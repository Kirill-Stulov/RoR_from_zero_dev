...
# переменная класса разделяется между самим классом, т.е к какому бы классу или подклассу мы не обратились, эта переменная будет иметь общее значение для них всех!
  ...
  @@stations = []                                                          # это нужно для главного меню и для railway_station.rb, они наследуют эту переменную класса из этого класса Train
  @@routes = []
  ...
  # @@inst = 0
    ...
    if valid? #== true                          #тоже добавлено в В railway_station !!!! ПРОВЕРКА   # valid? запускается при инициализации не смотря на if
      @@tr_names << self                                       # а можно класть просто self.name # каждый созданный поезд добавляется в массив @@tr_names (если прошел валидацию), потому в main мне уже не нужна конструкция @@tr_names << Train.new
