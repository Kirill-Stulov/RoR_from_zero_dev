...
class RailwayStation < Train         			# Наследуем из train, потому как используем его метод count_hash_dublicate
	...
	def initialize(name)             			# Имеет название, которое указывается при ее создании
		@name = name
		@trains = {}
		if self.valid?                               # валидация из modules
			...
			@@stations.map{ |obj| puts obj.name }                   						 	# имя станции вывел с помощью attr_accessor :name в шапке файла railway_station.rb   Читать можно тут: https://www.ruby-lang.org/ru/documentation/quickstart/3/
		else
			puts "Станция не создана!"          # если валидация станции не прошла, то выбрасывается исключение из raise модуля validate! (подробнее расписано в train.rb в initialize)
		end
