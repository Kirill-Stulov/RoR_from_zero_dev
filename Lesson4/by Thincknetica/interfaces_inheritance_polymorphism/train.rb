class Train
  ...
  def list                     # # возвращает номер поезда в качестве ключа, а тип поезда в качестве значения. И где это используется?
    return {@number => @type}
  end
...
# Методы вынесены потому что они не используются клиентским кодом, а только другими методами.
  ...
  def count_hash_dublicate(hash, value)  # этот метод используется в railway_station
    count = 0
