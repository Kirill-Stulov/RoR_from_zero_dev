...
#=end
	...
	while choice == 9                                  # задание для урока 5 - создать метод класса find, который принимает номер поезда и возвращает объект поезда по номеру или nil, если поезд с таким номером не найден.
		puts "1 -> Показать объект поезда по номеру"
		...
		if input == 1 && @@tr_names.empty?
			...
			num = gets.chomp.to_i
			Train.find(num)                               # метод класса find
		elsif input == 2
		break
		else
